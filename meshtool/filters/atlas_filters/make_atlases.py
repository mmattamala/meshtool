...
class TexcoordSet(object):
    ...
    def __eq__(self, other):
        return self.geom_id == other.geom_id and self.prim_index == other.prim_index and self.setnum == other.setnum
...
def getTexcoordToImgMapping(mesh):
    ...
    all_texcoords = dict((texset, imglist) for texset, imglist in all_texcoords.items() if len(imglist) > 0)
...
def packImages(mesh, img2texs, unique_images, image_scales):
    ...
    if not success:
        ...
        return combinePacks(
            packImages(mesh, img2texs, group1, image_scales), packImages(mesh, img2texs, group2, image_scales)
        )

    width = rp.width
    height = rp.height

    num_coords = sum([len(img2texs[imgpath]) for imgpath in unique_images])
    print(
        f"actually making atlas of size {width}x{height} with {len(unique_images)} subimages referenced by {num_coords} texcoords"
    )
...
def makeAtlases(mesh):
    ...
    for texset, imgpaths in tex2img.items():
        ...
        if len(imgpaths) == 1:
            texarray = (
                mesh.geometries[texset.geom_id].primitives[texset.prim_index].texcoordset[texset.texcoordset_index]
            )
...
def FilterGenerator():
    class MakeAtlasesFilter(OptimizationFilter):
        def __init__(self):
            super(MakeAtlasesFilter, self).__init__(
                "make_atlases",
                "Makes a texture atlas with the textures referenced in the "
