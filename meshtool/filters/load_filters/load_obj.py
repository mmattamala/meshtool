...
def loadMaterialLib(data, namer, aux_file_loader=None):
    ...
    illumination_map = collections.defaultdict(lambda: "blinn", **{0: "constant", 1: "lambert"})
    ...
    for line in file_like:
        ...
        if command == "newmtl":
            ...
            current_effect = collada.material.Effect(namer.name(line), [], "blinn")
    ...
    return {"material_map": material_map, "images": cimages, "effects": effects}
    ...
    for group in groups:
        ...
        if len(group.face_lengths) > 0:
            ...
            polylist = geom.createPolylist(
                face_indices, face_lengths, input_list, group.material or namer.name("nullmaterial")
            )
...
class OBJLoadFilter(LoadFilter):
    ...
    def apply(self, filename):
        ...
        col = loadOBJ(open(filename, "rb").read(), aux_file_loader=filepath_loader(filename))
