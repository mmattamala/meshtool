...
def seg_intersect(a1, a2, b1, b2):
    ...
    if (
        intersect[0] > max(a1[0], a2[0])
        or intersect[0] < min(a1[0], a2[0])
        or intersect[1] > max(a1[1], a2[1])
        or intersect[1] < min(a1[1], a2[1])
    ):
        return None
    return intersect
...
def uniqify_multidim_indexes(sourcedata, indices, return_map=False):
    unique_data, index_map = numpy.unique(
        sourcedata.view([("", sourcedata.dtype)] * sourcedata.shape[1]), return_inverse=True
    )
...
class SanderSimplify(object):
    def __init__(self, mesh, pmbuf):
        ...
        for boundgeom in chain(mesh.scene.objects("geometry"), mesh.scene.objects("controller")):
            ...
            for boundprim in boundgeom.primitives():
                ...
                bad_tris = (
                    (boundprim.vertex_index[:, 0] == boundprim.vertex_index[:, 1])
                ...
                self.all_vert_indices.append(
                    numpy.delete(boundprim.vertex_index, numpy.where(bad_tris), axis=0) + self.vertex_offset
                )
                self.all_normal_indices.append(
                    numpy.delete(boundprim.normal_index, numpy.where(bad_tris), axis=0) + self.normal_offset
                )
                ...
                if boundprim.texcoordset and len(boundprim.texcoordset) > 0:
                    ...
                    self.all_orig_uv_indices.append(
                        numpy.delete(texindex, numpy.where(bad_tris), axis=0) + self.uv_offset
                    )
                    self.uv_offset += len(texsource)
                else:
                    self.all_orig_uv_indices.append(
                        numpy.zeros(shape=(len(boundprim.index) - numpy.sum(bad_tris), 3), dtype=numpy.int32)
                    )
        ...
        self.all_orig_uvs = (
            numpy.concatenate(self.all_orig_uvs) if len(self.all_orig_uvs) > 0 else numpy.array([], dtype=numpy.float32)
        )
        ...
        self.all_orig_uv_indices = (
            numpy.concatenate(self.all_orig_uv_indices)
            if len(self.all_orig_uv_indices) > 0
            else numpy.array([], dtype=numpy.int32)
        )
    ...
    def build_vertex_graph(self):
        ...
        vertexgraph.add_nodes_from(
            ((edge[0], {facenum: True}) for facenum, edge in enumerate(self.all_vert_indices[:, (0, 1)]))
        )
        vertexgraph.add_nodes_from(
            ((edge[0], {facenum: True}) for facenum, edge in enumerate(self.all_vert_indices[:, (0, 2)]))
        )
        vertexgraph.add_nodes_from(
            ((edge[0], {facenum: True}) for facenum, edge in enumerate(self.all_vert_indices[:, (1, 2)]))
        )
        vertexgraph.add_nodes_from(
            ((edge[1], {facenum: True}) for facenum, edge in enumerate(self.all_vert_indices[:, (0, 1)]))
        )
        vertexgraph.add_nodes_from(
            ((edge[1], {facenum: True}) for facenum, edge in enumerate(self.all_vert_indices[:, (0, 2)]))
        )
        vertexgraph.add_nodes_from(
            ((edge[1], {facenum: True}) for facenum, edge in enumerate(self.all_vert_indices[:, (1, 2)]))
        )
    ...
    def build_face_graph(self):
        ...
        facegraph.add_nodes_from(
            (
                ...
                for i, tri in enumerate(self.all_vert_indices)
            )
        )

        self.invalid_edges = set()
        for (v1, v2) in self.vertexgraph.edges_items():
    ...
    def initialize_chart_merge_errors(self):
        ...
        for v1, v2 in self.facegraph.edges_items():
    ...
    def merge_charts(self):
        ...
        while len(self.merge_priorities) > 0:
            ...
            for otherface in adj_faces:
                if self.facegraph.node[otherface]["diffuse"] != diffuse:
                    continue
                otheredges = self.facegraph.node[otherface]["edges"]
    ...
    def update_corners(self, enforce=False):
        ...
        for face, facedata in self.facegraph.nodes_iter(data=True):
            ...
            corners = set(
                (v for v in vertices if self.facegraph.subgraph(self.vertexgraph.node[v].keys()).number_of_edges() >= 3)
            )
            self.facegraph.node[face]["corners"] = corners

        if enforce:
            for (face1, face2) in self.facegraph.edges_items():
                edges1 = self.facegraph.node[face1]["edges"]
                edges2 = self.facegraph.node[face2]["edges"]
                shared_edges = edges1.intersection(edges2)
    ...
    def create_initial_parameterizations(self):
        ...
        for (face, facedata) in self.facegraph.nodes_iter(data=True):
            ...
            if len(interior_verts) > 0:
                ...
                for edge in self.vertexgraph.subgraph(unique_verts).edges_items():
                    v1, v2 = edge
        ...
        for (face, facedata) in self.facegraph.nodes_iter(data=True):
            ...
            for iteration in range(1, 5):
                ...
                while len(vert_stretch_heap) > 0:
                    ...
                    neighborhood_L2, origA2d = stretch_metric(
                        neighborhood_tri3d, neighborhood_tri_2d, return_A2d=True, normalize=True
                    )
                    ...
                    for xval in numpy.arange(rangemin, rangemax, step):
                        ...
                        neighborhood_tri_2d = tri_2d[neighborhood_selector]
                        neighborhood_L2 = stretch_metric(
                            neighborhood_tri3d, neighborhood_tri_2d, flippedCheck=origA2d, normalize=True
                        )
    ...
    def resize_charts(self):
        ...
        for matnum, (i, mat) in enumerate(self.tri2material):
            ...
            if (
                mat is not None
                and mat.effect is not None
                and mat.effect.diffuse is not None
                and not isinstance(mat.effect.diffuse, tuple)
            ):
    ...
    def evaluate_edge_collapse(self, v1, v2):
        ...
        for moved_tri_idx in moved:
            ...
            vec1 = numpy.array(
                [other_uv_pts[1][0] - other_uv_pts[0][0], other_uv_pts[1][1] - other_uv_pts[0][1], 0],
                dtype=numpy.float32,
            )
            vec2_v1 = numpy.array(
                [v1_pt[0] - other_uv_pts[0][0], v1_pt[1] - other_uv_pts[0][1], 0], dtype=numpy.float32
            )
            vec2_v2 = numpy.array(
                [v2_pt[0] - other_uv_pts[0][0], v2_pt[1] - other_uv_pts[0][1], 0], dtype=numpy.float32
            )
            ...
            if v2_direction > 0 and not (v1_direction > 0) or v2_direction < 0 and not (v1_direction < 0):
                return
    ...
    def initialize_simplification_errors(self):
        ...
        for (vv1, vv2) in self.vertexgraph.edges_items():
    ...
    def simplify_mesh(self):
        ...
        while len(self.contraction_priorities) > 0:
            ...
            for t2, t2_idx in zip(v2tris, v2tri_idx):
                if v1 in t2_idx:
                    degenerate.add(t2)
                    self.simplify_operations.append(
                        (STREAM_OP.TRIANGLE_ADDITION, t2, t2_idx, self.all_normal_indices[t2], self.new_uv_indices[t2])
                    )
            ...
            for t2, t2_idx in zip(v2tris, v2tri_idx):
                if v1 not in t2_idx:
                    ...
                    self.simplify_operations.append(
                        (STREAM_OP.INDEX_UPDATE, t2, where_v2, prev_vertex_value, prev_normal_value, prev_uv_value)
                    )
    ...
    def enforce_simplification(self):
        ...
        while len(self.tris_left) > TRIANGLE_MAXIMUM:
            ...
            self.simplify_operations.append(
                (STREAM_OP.TRIANGLE_ADDITION, tri, tri_idx, self.all_normal_indices[tri], self.new_uv_indices[tri])
            )
    ...
    def pack_charts(self):
        ...
        for face_or_color, chartim in self.chart_ims.items():
            ...
            x, y, w, h, width, height = (
                float(i) for i in (x, y, w, h, self.chart_packing.width, self.chart_packing.height)
            )
    ...
    def split_base_and_pm(self):
        ...
        self.new_uvs, self.new_uv_indices, self.old2newuvmap = uniqify_multidim_indexes(
            self.new_uvs, self.new_uv_indices, return_map=True
        )
        ...
        stacked_indices = (
            numpy.hstack(
                (
                    self.all_vert_indices.reshape(-1, 1),
                    self.all_normal_indices.reshape(-1, 1),
                    self.new_uv_indices.reshape(-1, 1),
                )
            )
            .flatten()
            .reshape((-1, 3))
        ...
        unique_stacked_indices, index_map, new_tris = numpy.unique(
            stacked_indices.view([("", stacked_indices.dtype)] * stacked_indices.shape[1]),
            return_index=True,
            return_inverse=True,
        )
        unique_stacked_indices = unique_stacked_indices.view(stacked_indices.dtype).reshape(
            -1, stacked_indices.shape[1]
        )
        ...
        for operation in reversed(self.simplify_operations):
            if operation == STREAM_OP.OPERATION_BOUNDARY:
                ...
                newtri = []
                for pt in (
                    (vert_idx[0], norm_idx[0], uv_idx[0]),
    ...
    def save_mesh(self):
        ...
        sander_contributor = collada.asset.Contributor(
            authoring_tool="meshtool",
    ...
    def simplify(self):
        ...
        if (
            self.orig_tri_count < TRIANGLE_MINIMUM
            or float(self.orig_tri_count - self.base_tri_count) / self.orig_tri_count < STREAM_THRESHOLD
        ):
...
def FilterGenerator():
    class SandlerSimplificationFilter(SimplifyFilter):
        def __init__(self):
            super(SandlerSimplificationFilter, self).__init__(
                "sander_simplify", "Simplifies the mesh based on sandler, et al. method."
            )
