...
def splitTriangleTexcoords(mesh):
    ...
    for geom in mesh.geometries:
        ...
        for prim_index, prim in enumerate(geom.primitives):
            ...
            tris2keep_idx = (
                numpy.apply_along_axis(numpy.sum, 1, numpy.apply_along_axis(numpy.sum, 2, texarray > 2.0)) == 0
            )
            ...
            while len(index2split) > 0 and not giveup:
                ...
                if len(orig_index) - len(prim.index) > max(
                    MAX_TRIANGLE_INCREASE, len(prim.index) * MAX_TRIANGLE_MULTIPLIER
                ):
...
def FilterGenerator():
    class SplitTriangleTexcoordsFilter(OptimizationFilter):
        def __init__(self):
            super(SplitTriangleTexcoordsFilter, self).__init__(
                "split_triangle_texcoords",
                "Splits triangles that span multiple texcoords into multiple triangles to better help texture atlasing",
            )
