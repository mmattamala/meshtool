...
def combinePrimitives(mesh):
    ...
    for geometry in mesh.geometries:
        ...
        for primitive in geometry.primitives:
            ...
            if not matched:
                primitive_sets.append({"bindings": bindings, "inputs": input_list, "members": [primitive]})
...
def FilterGenerator():
    class CombinePrimitivesFilter(OptimizationFilter):
        def __init__(self):
            super(CombinePrimitivesFilter, self).__init__(
                "combine_primitives",
                "Combines primitives within a geometry if they have the same sources and scene material mapping (triangle sets only)",
            )
