...
def stripUnusedSources(mesh):
    for geom in mesh.geometries:
        ...
        for prim in geom.primitives:
            for semantic, inputs in prim.sources.items():
                for offset, semantic, srcid, setid, src in inputs:
                    srcid_count[srcid[1:]] += 1
        for srcid, count in srcid_count.items():
            if count == 0:
...
def FilterGenerator():
    class StripUnusedSourcesFilter(OptimizationFilter):
        def __init__(self):
            super(StripUnusedSourcesFilter, self).__init__(
                "strip_unused_sources",
                "Strips any source arrays from geometries if they aren't referenced by any primitives",
            )
