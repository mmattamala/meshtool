...
def optimizeSources(mesh):
    ...
    for geom in mesh.geometries:
        ...
        for semantic, sources in src_by_semantic.items():
            ...
            unique_data, index_locs = numpy.unique(
                new_data.view([("", new_data.dtype)] * new_data.shape[1]), return_inverse=True
            )
...
def FilterGenerator():
    class OptimizeSourcesFilter(OptimizationFilter):
        def __init__(self):
            super(OptimizeSourcesFilter, self).__init__(
                "optimize_sources", "Compresses sources to unique values, updating triangleset indices"
            )
