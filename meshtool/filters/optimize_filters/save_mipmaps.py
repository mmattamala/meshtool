        ...
        for prop in effect.supported:
            ...
            if isinstance(propval, collada.material.Map):
                ...
                for i, pil_img in enumerate(pil_images):
                    ...
                    byte_ranges.append(
                        {"offset": file_start, "length": file_len, "width": pil_img.size[0], "height": pil_img.size[1]}
                    )
...
def FilterGenerator():
    class SaveMipMapsFilter(OptimizationFilter):
        def __init__(self):
            super(SaveMipMapsFilter, self).__init__(
                "save_mipmaps",
                "Saves mipmaps to disk in tar format in the same location as textures but with an added .tar. The archive will contain PNG or JPG images.",
            )
