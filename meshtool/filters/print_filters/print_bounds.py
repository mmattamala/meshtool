...
def v3_array_pt_dist(arr, pt):
    ...
    return numpy.sqrt(
        numpy.square(arr[:, 0] - pt[0]) + numpy.square(arr[:, 1] - pt[1]) + numpy.square(arr[:, 2] - pt[2])
    )
...
def printBoundsInfo(mesh):
    ...
    print(f"Bounds: <{fmtpt(boundsinfo['bounds'][0])}, {fmtpt(boundsinfo['bounds'][1])}>")
    print(f"Center: {fmtpt(boundsinfo['center'])}")
    print(
        f"Point farthest from center: {fmtpt(boundsinfo['center_farthest'])} at distance of { boundsinfo['center_farthest_distance']:.7g}"
    )
