...
def getPmPerceptualError(mesh, pm_filebuf, mipmap_tarfilebuf):
    ...
    if pm_filebuf is not None:
        ...
        while len(data) > 0:
            (refinements_read, num_refinements, pm_refinements, data_left) = pdae_utils.readPDAEPartial(
                data, refinements_read, num_refinements
            )
    ...
    try:
        ...
        hprs = [(0, 0, 0), (0, 90, 0), (0, 180, 0), (0, 270, 0), (90, 0, 0), (-90, 0, 0)]
...
def FilterGenerator():
    class PrintPmPerceptualError(PrintFilter):
        def __init__(self):
            super(PrintPmPerceptualError, self).__init__(
                "print_pm_perceptual_error",
                "Prints perceptual error at different levels of a progressive mesh compared to full resolution",
            )

            self.arguments.append(
                FileArgument("pm_file", "Path of the progressive mesh file. Specify NONE if no pm file.")
            )
