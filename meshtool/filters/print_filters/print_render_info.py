...
def getSceneInfo(mesh):
    ...
    for boundobj in itertools.chain(scene.objects("geometry"), scene.objects("controller")):
        ...
        for boundprim in boundgeom.primitives():
            ...
            num_texcoords += (
                boundprim.texcoord_indexset[0].size
                if boundprim.texcoord_indexset is not None and len(boundprim.texcoord_indexset) > 0
                else 0
            )
    ...
    return (
        num_triangles,
        num_draw_raw,
        num_draw_with_instances,
        num_draw_with_batching,
        num_lines,
        num_vertices,
        num_normals,
        num_texcoords,
...
def getRenderInfo(mesh):
    ...
    return {
        "texture_ram": getTextureRAM(mesh),
...
def printRenderInfo(mesh):
    ...
    print(f"Total texture RAM required: { humanize_bytes(render_info['texture_ram'])}")
    print(f"Total triangles: {render_info['num_triangles']}")
    print(f"Total vertices: {render_info['num_vertices']}")
    print(f"Total normals: {render_info['num_normals']}")
    print(f"Total texcoords: {render_info['num_texcoords']}")
    print(f"Raw number of draw calls: {render_info['num_draw_raw']}")
    print(f"Number of draw calls with instance batching: {render_info['num_draw_with_instances']}")
    print(f"Number of draw calls with instance and material batching: {render_info['num_draw_with_batching']}")
    print(f"Number of lines: {render_info['num_lines']}")


def FilterGenerator():
    class PrintRenderInfoFilter(PrintFilter):
        def __init__(self):
            super(PrintRenderInfoFilter, self).__init__(
                "print_render_info",
                "Prints estimated number of batches, total number of triangles, and total texture memory",
            )
