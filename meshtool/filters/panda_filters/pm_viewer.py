...
class PandaPmViewer:
    def __init__(self, mesh, pm_filebuf):
        ...
        self.slider = DirectSlider(
            range=(0, len(self.pm_refinements)),
            value=0,
        ...
        self.triText = OnscreenText(
            text="", pos=(-1, 0.85), scale=0.15, fg=(1, 0.5, 0.5, 1), align=TextNode.ALeft, mayChange=1
        )
...
def FilterGenerator():
    class PmViewer(VisualizationFilter):
        def __init__(self):
            super(PmViewer, self).__init__(
                "pm_viewer", "Uses panda3d to bring up a viewer of a base mesh and progressive stream"
            )
        ...
        def apply(self, mesh, pm_filename):
            try:
                ...
                print(f"Error opening pm file: {str(ex)}")
                sys.exit(1)
