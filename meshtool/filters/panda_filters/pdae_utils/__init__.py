...
class PDAETest(unittest.TestCase):
    def testLoad(self):
        for i in range(2):
            ...
            after = time.time()
            print(f"Took {after - start} seconds to load")
    ...
    def testPartial(self):
        ...
        for test_file in to_test:
            ...
            KB = 1024
            MB = KB * KB
            test_block_sizes = [100 * B, 500 * B, 1 * KB, 5 * KB, 50 * KB, 100 * KB, 1 * MB, 2 * MB, 5 * MB]
            ...
            for block_size in test_block_sizes:
                ...
                all_pm_refinements = []
                while len(curdata) > 0:
                    (refinements_read, num_refinements, pm_refinements, data_left) = readPDAEPartial(
                        curdata, refinements_read, num_refinements
                    )
