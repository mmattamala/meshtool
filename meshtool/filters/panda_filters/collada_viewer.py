...
def runViewer(mesh):
    ...
    for boundlight in mesh.scene.objects("light"):
        ...
        if isinstance(boundlight, collada.light.BoundDirectionalLight):
            ...
            lightNP.setPos(Vec3(boundlight.position[0], boundlight.position[1], boundlight.position[2]))
            lightNP.lookAt(
                Point3(boundlight.direction[0], boundlight.direction[1], boundlight.direction[2]),
    ...
    for boundcam in mesh.scene.objects("camera"):
        if isinstance(boundcam, collada.camera.BoundPerspectiveCamera):
            ...
            if boundcam.xfov is not None and boundcam.yfov is not None:
                ...
                xfov = math.degrees(
                    2.0 * math.atan(boundcam.aspect_ratio * math.tan(math.radians(0.5 * boundcam.yfov)))
                )
                ...
                # aspect_ratio = tan(0.5*xfov) / tan(0.5*yfov)
                xfov = math.degrees(
                    2.0 * math.atan(base.camLens.getAspectRatio() * math.tan(math.radians(0.5 * boundcam.yfov)))
                )
                base.camLens.setFov(xfov, boundcam.yfov)
            ...
            base.camera.lookAt(
                Point3(boundcam.direction[0], boundcam.direction[1], boundcam.direction[2]),
...
def FilterGenerator():
    class ColladaViewer(VisualizationFilter):
        def __init__(self):
            super(ColladaViewer, self).__init__(
                "collada_viewer", "Uses panda3d to bring up a viewer with lights and camera from the collada file"
            )
