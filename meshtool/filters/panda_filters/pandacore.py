...
def getPrimAndDataFromTri(triset, matstate):
    ...
    if (
        needsTanAndBin
        and isinstance(triset, collada.triangleset.TriangleSet)
        and len(triset.texcoordset) > 0
        and len(triset.textangentset) == 0
    ):
    ...
    vdata, indexdata = getVertexData(
        ...
        triset.normal_index,
        triset.texcoordset,
        triset.texcoord_indexset,
        triset.textangentset,
        triset.textangent_indexset,
        triset.texbinormalset,
    ...
    if prim_material and prim_material.effect:
        ...
        if isinstance(diffuse, collada.material.Map) or isinstance(transparent, collada.material.Map):
            ...
            diffuseTexture = getTexture(
                color=diffuseMap, alpha=transparentMap, texture_cache=texture_cache, diffuseinit=diffuseInitColor
            )
        ...
        if col_inst and col_inst.assetInfo:
            for contributor in col_inst.assetInfo.contributors:
                ...
                if len(split) == 3 and split[0].strip().lower() == "google" and split[1].strip().lower() == "sketchup":
...
def centerAndScale(nodePath):
    ...
    if not nodePath.isEmpty():
        ...
        if not boundingSphere.isEmpty():
            ...
            nodePath.setPos(-1 * centerPt.getX(), -1 * centerPt.getY(), -1 * centerPt.getZ())
